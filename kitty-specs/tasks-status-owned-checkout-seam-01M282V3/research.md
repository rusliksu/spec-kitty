...
## Дополнительная приёмка 13 сентября 2026 года

**D-20 — ревью и финализация теряли выбранный корень глубже обработчика команды.**
Новые сценарии на реальных Git worktree обнаружили чтение подзадач и текущего
статуса из primary, отсутствие опции у `status validate/lifecycle`, нулевой состав
задач при финализации и неверное место сохранения документа ревью.
Корень передаётся через существующие read/write interfaces до канонических
журнала, размещения артефакта и commit router; прежние вызовы без опции сохранены.

**D-21 — ревью single_branch не должно получать фиктивный workspace.**
В исходной миссии нет отдельной lane реализации. Независимое архитектурное ревью
подтвердило проверку реального выбранного checkout по U..H: U — настроенный
upstream основной ветки, H — HEAD целевой ветки. Отсутствующие данные и изменение
HEAD вызывают отказ. Изменения только планирования не считаются реализацией.
Отдельная проверка устранила старый fail-open при неразрешимом WP.

**D-22 — предупреждение о двух версиях миссии отсутствовало.**
Добавлена диагностика `owned_mission_divergence` в stderr. Для поиска обоих каталогов
используется канонический placement seam; архитектурный тест первоначально отверг
ручное составление пути, после исправления проходит без новой allowlist.

**D-23 — исходные результаты тестов ревью также должны быть локальны выбранной копии.**
Независимое ревью обнаружило повторное чтение `baseline-tests.json` из primary.
Проверка с различными результатами в двух копиях подтверждает выбор owned baseline
и непустой набор реальных изменённых исходников для pre-review gate.

Точные команды, положительные и отрицательные результаты собраны в
`traces/owned-checkout-seam-evidence.md`. Одобрение и приёмка фиксируются только
после завершения проверок; прежние события не переписываются.

**D-24 — CI потребовал проверять состав документов, а не только их количество.**
В прогоне `34748862869` архитектурный шард 1 обнаружил новый golden-count assertion:
проверка считала документы ревью, вместо того чтобы проверять ожидаемый документ.
Assertion заменён сравнением имён с `review-cycle-1.md`; чтение его содержимого
из Git и проверки основной копии сохранены. Штатная архитектурная проверка и
весь новый набор single_branch проходят: `24 passed`,
`out/golden-count-remediation.xml`. Allowlist и frozen baseline не менялись.

**D-25 — полный CI выявил ещё две несовместимости вызовов без opt-in.**
В `fast-tests-cli` прогона `34749402998` неизменённые проверки обнаружили:
новая основа реализации ошибочно использовалась без `effective_root`, а отсутствие
поля `owned_checkout` в прежнем контексте маскировало ошибку записи `disk full`.
Оба падения повторены локально (`out/ci-compatibility-red.xml`).

Выбор новой основы теперь возможен только при объявленном корне и непустом строковом
значении. В адаптере записи отсутствие нового поля означает прежний вызов без opt-in;
ошибки записи по-прежнему распространяются. Старые тесты не изменены.
Оба их файла и новые single_branch-сценарии проходят: **42 passed**,
`out/ci-compatibility-green.xml`; Ruff прошёл, новых сообщений mypy нет.

Независимый ревьюер отозвал одобрение версии `98a156e8` и выдал
`changes_requested` по ссылке на CI. Это записано штатными переходами:
`01M2D39R629MZCZ807HTE3BH9K` (`in_review`) и
`01M2D3BKHY4C5VFECYNBM9ZAVR` (`in_progress` с результатом ревью).
T006 повторно открыта новым событием; прежние записи не переписывались.

**D-26 — обязательная таблица issues обнаружена штатным approval gate.**
Переход первой задачи в approved отказал из-за отсутствия issue-matrix для issue 26
и процитированного upstream 3328. Добавлена предметная таблица по документированной
схеме; upstream issue проверен закрытым, существующая граница и её тесты подтверждены
в исходной базе. Валидатор и независимое ревью прошли, обычный повтор approval успешен.
Это исправление недостающего артефакта; status/event history вручную не редактировались.
