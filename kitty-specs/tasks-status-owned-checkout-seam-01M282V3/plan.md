...
Итоговая реализация сохраняет один явный выбор checkout и существующий механизм записи.
При приёмке обнаружились пропуски передачи корня в проверке подзадач, финализации,
чтении текущего состояния и сохранении ревью. Эти вызовы используют тот же `effective_root`.
Два небольших модуля выделяют проверку реальной основы Git и диагностику расхождения
копий, чтобы не помещать дополнительную логику в уже крупные обработчики команд.
Новая таблица состояния или альтернативный механизм записи не вводятся.

### Уточнение реализации от 13 сентября 2026 года

Ревью `single_branch` проверяет реальную реализацию в существующем task checkout:
настроенный upstream основной ветки должен быть предком HEAD целевой ветки.
Этот диапазон используется для проверки изменённых файлов; отсутствие доказанной
основы или изменение HEAD перед записью блокирует переход. Для этого не создаются
фиктивные lane/workspace context и не переписывается история ранних переходов.
Подробные условия и наблюдаемые результаты описаны в `contracts/owned-checkout.md`.

Состав `owned_files` синхронизирован с фактическими зависимостями. Вторая задача
исправляет рабочий код команд, поэтому её режим исправлен с `planning_artifact`
на `code_change`: к ней применяются проверки реальной реализации. Новые требования,
продуктовые сценарии и право на merge/release этим уточнением не добавляются.
