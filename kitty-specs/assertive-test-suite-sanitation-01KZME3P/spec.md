...
1. **Given** a deterministic, current, live-entry-point reproduction of an accepted product defect, **when** it is evaluated, **then** the product is fixed with red-to-green proof before that reproduction joins required CI, or one executable issue-linked diagnostic remains outside required CI until the product is fixed; required CI is never deliberately red.
...
| FR-002 | Disposition rubric | Assign each candidate a disposition: `KEEP`, `CONSOLIDATE`, `FIX_TEST`, `FIX_PRODUCT`, `DELETE`, or time-bounded `TEMPORARY`. `FIX_TEST` and `FIX_PRODUCT` are nonterminal until the repair executes and receives `KEEP`, or evidence converts the candidate to another allowed terminal disposition; `CONSOLIDATE` is terminal only after the surviving guard and removed members are recorded. | High | Approved |
| FR-003 | Causal survival proof | Retain a test only when node-level or valid family evidence shows a plausible production fault, incompatible consumed contract shape, or known-bad live entry point reaches the product action and makes the intended oracle fail. Faults must violate a named current authority; collection/import/setup failures and scanner-self-tests do not count. | High | Approved |
...
| FR-009 | Honest regression lane | For each accepted open P0, preserve exactly one executable live-entry-point reproduction: after red-to-green proof it runs and passes in required CI as the retained guard; while unresolved it remains an issue-linked diagnostic outside required CI and does not count as retained suite protection. The reproduction may be retired only when the issue is formally closed, downgraded, or adjudicated invalid. It never skips, xfails, quarantines, or retries. | High | Approved |
...
| FR-014 | Disposition ledger | Produce a machine-readable ledger with candidate-unit identity, scope and family/cluster basis, evidence, verdict, deletion/retention action, surviving guard, issue, owner, and expiry where applicable. Every `KEEP` must reference node-level proof or a named valid family proof; every outcome divergence expands to node-level rows. | High | Approved |
...
| NFR-002 | Deletion evidence coverage | 100% of deletions record caller/authority/routing/overlap evidence plus a sensitivity or non-causality result at the smallest unit whose members share path, oracle, and outcome; no deletion relies only on age, name, marker, directory, or runtime. | Reliability | High | Approved |
...
| NFR-004 | Known-red preservation | Define the known-red set as exact nodeid+outcome records from the immutable planning base after the minimal #3283 fix is replayed in a disposable base worktree. The final set is identical unless this mission fixes the product and proves red-to-green through the same live entry point; raw pre-fix infrastructure results remain separate. | Integrity | High | Approved |
...
| NFR-007 | No causal coverage regression | Unique live-contract and plausible-authority-violating fault coverage must not decrease; focused mutation/fault probes for each changed cluster must preserve all non-equivalent kills owned by deleted tests. Every `KEEP` has node proof or a valid family proof over the same production path, oracle, and parameterized boundary. | Quality | High | Approved |
...
| NFR-009 | Bounded temporary states | Every temporary exemption expires within 30 days and names an owner, issue, and terminal action; zero expired exemptions may remain. | Governance | Medium | Approved |
...
| C-007 | Issue traceability | Referenced issues #1931, #2309, #2316, #2342, #2645, #2782, #3184, and #3283 must have explicit issue-matrix verdicts before merge. | Tracking | High | Approved |
...
| C-009 | Required CI stays green | No unresolved product-defect reproduction may make required CI deliberately red. It must be fixed before admission or remain an issue-linked, executable non-required diagnostic that is excluded from retained-suite protection claims. | Governance | High | Approved |
...
- **SC-002**: Every deletion has a complete evidence bundle, every `KEEP` has node-level or valid family causal proof, and review finds zero deleted unique live-contract or plausible-authority-violating fault guards.
