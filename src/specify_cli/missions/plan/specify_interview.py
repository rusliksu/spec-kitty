"""Specify interview flow with Widen Mode affordance (FR-002, WP08 T043).

Applies the same ``[w]iden`` pattern as the charter interview to the
``specify`` interview surface.  Each question opened here records a Decision
Moment with ``origin_flow=SPECIFY``.

Public API:
    run_specify_interview(questions, repo_root, mission_slug, console, actor)
...
infrastructure already used by charter.py.
"""
...
def run_specify_interview(  # noqa: C901
    ...
    """Drive a specify interview with the Widen Mode affordance.
    ...
    for question_id, question_text in questions:
        ...
        with contextlib.suppress(_DecisionError, Exception):
            dm_response = _dm_service.open_decision(
                ...
                origin_flow=_DmOriginFlow.SPECIFY,
                step_id=f"specify.{question_id}",
        ...
        while True:
            ...
            if (
                ...
                if result.action == WidenAction.BLOCK:
                    # Enter blocked-prompt loop (reuse charter helper pattern)
                        ...
                        try:
                            widen_store.add_pending(
                                WidenPendingEntry(
                                    decision_id=result.decision_id or current_decision_id,
                                    mission_slug=mission_slug,
                                    question_id=f"specify.{question_id}",
                                    question_text=question_text,
            ...
            if user_answer.strip().lower() == "!cancel":
                with contextlib.suppress(Exception):
                    _dm_service.cancel_decision(
                        ...
                        mission_slug=mission_slug,
                        decision_id=current_decision_id,
                        rationale="owner canceled during specify interview",
                        actor=actor,
                    )
                ...
                with contextlib.suppress(Exception):
                    _dm_service.defer_decision(
                        ...
                        mission_slug=mission_slug,
                        decision_id=current_decision_id,
                        rationale="owner deferred during specify interview",
                        actor=actor,
                    )
